...
```{r}
test_null8 = read_csv(here::here("03_generated_data", "full_csv", "test_null8.csv")) %>% 
  filter(generation > 25) %>% 
  group_by(lat, lon, generation) %>% 
  summarise(geno_pop_sum = sum(pop)) %>% 
  mutate(type = "null8")

test_mean8 = read_csv(here::here("03_generated_data", "full_csv", "test_mean8.csv")) %>% 
  filter(generation > 25) %>% 
  group_by(lat, lon, generation) %>% 
  summarise(geno_pop_sum = sum(pop)) %>% 
  mutate(type = "mean8")

test_enso8 = read_csv(here::here("03_generated_data", "full_csv", "test_enso8.csv")) %>% 
  filter(generation > 25) %>% 
  group_by(lat, lon, generation) %>% 
  summarise(geno_pop_sum = sum(pop)) %>% 
  mutate(type = "enso8")

test_shock8 = read_csv(here::here("03_generated_data", "full_csv", "test_shock8.csv")) %>% 
  filter(generation > 25) %>% 
  group_by(lat, lon, generation) %>% 
  summarise(geno_pop_sum = sum(pop)) %>% 
  mutate(type = "shock8") 
...
rm(data1,data2,data3,data4,data5,data6,test_shock_noevo,test_enso_noevo,test_mean_noevo,test_null_noevo,test_null8,test_mean8,test_enso8,test_shock8)
