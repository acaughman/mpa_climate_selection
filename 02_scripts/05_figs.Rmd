...
line_df_lat <- data %>%
  filter(generation > 25) %>%
  filter(evolution == "Yes") %>% 
  group_by(generation, mpa_size, climate, lat) %>%
  summarise(
    location_sum = sum(geno_pop_mean, na.rm = TRUE),
    max_temp = max(max_temp),
    min_temp = min(min_temp),
  ...
  mutate(mpa = case_when(
    mpa_size == "Small" & lat %in% c(50) ~ "MPA",
    mpa_size == "Medium" & lat %in% c(49:51) ~ "MPA",
    mpa_size == "Large" & lat %in% c(46:54) ~ "MPA",
    TRUE ~ "No MPA"
  ))
...
collapse = line_df %>% 
  ...
  filter(location_sum <= 100*100*20*2*.001) %>% 
...
p3 = ggplot(line_df_lat, aes(generation, location_sum)) +
  geom_line(aes(color = lat, linetype = mpa)) +
...
ggsave(p3, file = paste0("mpa_benefit_summary.pdf"), path = here::here("04_figs"), height = 8, width = 15)
```
...
p6 = ggplot(mean_change, aes(lat, mean_change)) +
  ...
  theme_bw() +
  ggh4x::facet_grid2(mpa_size ~ climate, scales = "free", independent = "y") + 
  theme(panel.grid.minor = element_blank(),
        strip.text = element_text(face = "bold"),
        strip.background = element_rect(fill = "white"),
