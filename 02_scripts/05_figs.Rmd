...
```{r}
p5 = ggplot(sum_climate8_f, aes(generation, pop_mean)) +
...
#ggsave(p5, file=paste0("test_mpa.pdf"), path = here::here("figs", "test"), height = 8, width = 8)
```

```{r}
p6 = ggplot(sum_climate8_f, aes(generation, pop_mean)) +
  geom_line(aes(color = type)) +
...
```{r}
p7 = ggplot(sum_climate8, aes(generation, pop_mean)) +
  geom_line(aes(color = type)) +
  facet_wrap(~cat, scales = "free_y", ncol = 1) +
  theme_bw() +
  labs(x = "Year", 
...
#ggsave(p7, file=paste0("test.pdf"), path = here::here("figs", "test"), height = 11, width = 8)
```
