...
p13 = ggplot(geno_lat_rate, aes(generation, mpa_mean_freq)) +
  geom_line(aes(color = as.factor(climate_rate))) +
  facet_grid(mpa_size~climate, scales = "free_y") +
  theme_bw() +
  labs(
    x = "",
    y = "Allele Frequency",
...
```{r}
p14 = ggplot(line_df_mpa_full_fp, aes(generation, location_sum)) +
  geom_vline(data = collapse_full_fp, aes(xintercept = min_gen, color = fishing_pressure, linetype = evolution), alpha = 0.8) +
  geom_hline(data = collapse_full_fp, aes(yintercept = collapse_point), alpha = 1, color = "darkgrey") +
  geom_line(aes(color = climate, linetype = evolution)) +
  facet_wrap(climate ~ mpa_size, scales = 'free', nrow = 3) +
  theme_bw() +
  labs(
    x = "Year",
    y = "Population Density",
...
ggsave(p14, file = paste0("decline_mpa_fishing_pressure.pdf"), path = here::here("04_figs"), height = 8, width = 15)
```
