...
```{r}
f = ggplot(point_sum, aes(generation, avg_diff)) +
...
ggsave(f, file = paste0("figX.pdf"), path = here::here("04_figs"), height = 8, width = 10)
```
...
shock_change = mean_change2 %>% 
  filter(climate == "Shocks") 

s6 = ggplot() +
  geom_vline(data = shock_years, aes(xintercept = generation), linetype = "dotted", alpha = 0.3) +
  geom_line(data = shock_change, aes(generation, mean_change, color = mpa_size, linetype = evolution)) +
  theme_bw() +
  labs(
    x = "Year",
    y = "Rate of Population Change",
...
ggsave(s6, file = paste0("shocks_rate.pdf"), path = here::here("04_figs", "supp"), height = 6, width = 15)
```
