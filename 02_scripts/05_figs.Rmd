...
```{r}
output_df = read_csv(here::here("tests", "mean_dyn.csv"))
```
...
ggsave(plot, file=paste0("mean_dyn_h.pdf"), path = here::here("tests"), height = 11, width = 8)
```
...
ggsave(p3, file=paste0("mean_dyn.pdf"), path = here::here("tests"), height = 11, width = 8)
```
