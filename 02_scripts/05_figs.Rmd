...
collapse = line_df %>% 
  ...
  filter(location_sum <= 100*2*.1*100*20) %>% 
...
collapse_full = line_df_mpa_full %>% 
  select(generation, evolution, mpa_size, climate, location_sum) %>% 
  filter(generation > 50) %>% 
  filter((location_sum <= (100*2*.1*3*3) & mpa_size == "Small") | (location_sum <= (100*2*.1*6*6) & mpa_size == "Medium") | (location_sum <= (100*2*.1*10*10) & mpa_size == "Large")) %>% 
  group_by(evolution, mpa_size, climate) %>%
  summarize(min_gen = min(generation))

collapse_point = data.frame(mpa_size = c("Small", "Medium", "Large"), collapse_point = c(100*2*.1*3*3, 100*2*.1*6*6, 100*2*.1*10*10))
collapse_point_full = data.frame(mpa_size = c("Small", "Medium", "Large"), collapse_point = c(100*2*.1*100*20, 100*2*.1*100*20, 100*2*.1*100*20))
```

```{r}
p1 = ggplot(line_df, aes(generation, location_sum)) +
  geom_vline(data = collapse, aes(xintercept = min_gen, color = climate, linetype = evolution), alpha = 0.8) +
  geom_hline(data = collapse_point_full, aes(yintercept = collapse_point), alpha = 1, color = "darkgrey") +
...
ggsave(p1, file = paste0("decline_full_10.pdf"), path = here::here("04_figs"), height = 8, width = 15)
```
...
ggsave(p7, file = paste0("collapse_points_10.pdf"), path = here::here("04_figs"), height = 8, width = 8)
```
...
ggsave(p10, file = paste0("decline_mpa_full_10.pdf"), path = here::here("04_figs"), height = 8, width = 15)
```
