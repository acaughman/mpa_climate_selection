...
df1 <- read_csv(here::here("sensitivity_analysis", "movement_rate", "mean_shock_med_sum.csv")) %>%
  mutate(climate = "Mean Shock") %>%
  mutate(dd = as.factor(dd)) %>%
  mutate(max_temp = as.character(max_temp))

df2 <- read_csv(here::here("sensitivity_analysis", "movement_rate", "mean_med_sum.csv")) %>%
  mutate(climate = "Mean") %>%
  mutate(dd = as.factor(dd)) %>%
  mutate(max_temp = as.character(max_temp))

df3 <- read_csv(here::here("sensitivity_analysis", "movement_rate", "shock_med_sum.csv")) %>%
  mutate(climate = "Shock") %>%
  mutate(dd = as.factor(dd)) %>%
  mutate(max_temp = as.character(max_temp))

df4 <- read_csv(here::here("sensitivity_analysis", "movement_rate", "enso_med_sum.csv")) %>%
  mutate(climate = "Enso") %>%
  mutate(dd = as.factor(dd)) %>%
  mutate(max_temp = as.character(max_temp))

df5 <- read_csv(here::here("sensitivity_analysis", "movement_rate", "null_med_sum.csv")) %>%
  mutate(climate = "Null") %>%
  mutate(dd = as.factor(dd)) %>%
  mutate(max_temp = as.character(max_temp))
...
ggsave(reposition_legend(p, 'center', panel='panel-3-2'), file = paste0("med.pdf"), path = here::here("sensitivity_analysis", "movement_rate"), height = 8, width = 15)
...
# ggsave(p, file = paste0("med.pdf"), path = here::here("sensitivity_analysis", "movement_rate"), height = 8, width = 15)
