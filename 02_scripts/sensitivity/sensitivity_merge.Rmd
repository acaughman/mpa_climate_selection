...
```{r}
df1 <- read_csv(here::here("sensitivity_analysis", "movement_rate", "large_enso_2_1.csv"))

df2 <- read_csv(here::here("sensitivity_analysis", "movement_rate", "enso_large_sum.csv"))
```
...
### large
#lon %in% c(6,7,8, 9, 10, 11, 12, 13, 14)
#lat %in% c(46,47,48, 49, 50, 51, 52, 53,54)

## SELECT ADULTS ONLY <-edit to change
mpa_df1 <- output_sum1 %>%
  filter(lon %in% c(6,7,8, 9, 10, 11, 12, 13, 14)) %>%
  filter(lat %in% c(46,47,48, 49, 50, 51, 52, 53,54)) %>%
  group_by(generation) %>%
  summarise(
    location_sum = sum(geno_pop_mean),
    max_temp = max(max_temp),
...
write_csv(data, here::here("sensitivity_analysis", "movement_rate", "enso_large_sum.csv"))
```
