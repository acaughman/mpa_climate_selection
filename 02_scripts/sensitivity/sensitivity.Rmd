...
```{r}
df1 <- read_csv(here::here("sensitivity_analysis", "initial_SST", "mean_shock_small_neg5.csv"))

df2 <- read_csv(here::here("sensitivity_analysis", "initial_SST", "mean_shock_small_neg1.csv")) 

df3 <- read_csv(here::here("sensitivity_analysis", "initial_SST", "mean_shock_small0.csv"))

df4 <- read_csv(here::here("sensitivity_analysis", "initial_SST", "mean_shock_small_pos5.csv"))

df5 <- read_csv(here::here("sensitivity_analysis", "initial_SST", "mean_shock_small_pos1.csv"))

df6 <- read_csv(here::here("sensitivity_analysis", "initial_SST", "mean_shock_small_pos2.csv"))
```
...
output_sum6 <- full_join(geno_mean6, pop_mean6) %>%
  mutate(freq = geno_pop_mean / pop_mean)

## SELECT GENERATIONS and ADULTS ONLY <-edit to change
plot_sum6 <- output_sum6 %>%
  filter(generation %in% c(25, 50, 75, 100, 125, 150, 175)) %>%
  mutate(generation = as.numeric(generation))

## SELECT ADULTS ONLY <-edit to change
line_df6 <- output_sum6 %>%
  group_by(generation) %>%
  summarise(
    location_sum = sum(geno_pop_mean),
    max_temp = max(max_temp),
...
data = full_join(data, mpa_df6) %>% 
  mutate(dd = as.factor(dd))

write_csv(data, here::here("sensitivity_analysis", "initial_SST", "mean_shock_small_sum.csv"))
```
