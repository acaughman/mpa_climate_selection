...
data8 = data %>% 
  filter(type %in% c("null8", "mean8", "enso8", "shock8", "null_noevo"))
...
```{r}
p8 = ggplot(sum_climate8, aes(generation, pop_mean)) +
...
p5 = ggplot(sum_climate5, aes(generation, pop_mean)) +
  geom_line(aes(color = type)) +
  theme_bw() +
  labs(x = "Year", 
       y = "Population Density", 
