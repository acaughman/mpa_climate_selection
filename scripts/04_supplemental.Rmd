...
by_lat = data %>% 
  ...
  mutate(lat_group = case_when(
    ...
    lat %in% c(81:90) ~ 9,
    lat %in% c(91:100) ~ 10
  )) %>%  # make generation numeric
  mutate(loc = case_when(
    lat_group %in% c(5,6) ~ "MPA",
    !lat_group %in% c(5,6) ~ "non-MPA",
  )) %>% 
  group_by(generation, evolution, mpa_size, climate, lat_group) %>% 
  summarise(
    location_sum = sum(pop_mean, na.rm = TRUE), # sum across location
    max_temp = max(max_temp),
    min_temp = min(min_temp)
...
collapse_by_lat <- by_lat %>%
  select(generation, evolution, mpa_size, climate, location_sum, lat_group) %>%
  filter(generation > 30) %>% 
  filter(location_sum <= 100 * 2 * 0.05 * 20) %>%  
  group_by(evolution, mpa_size, climate, lat_group) %>%
...
s6 <- ggplot(med2, aes(generation, location_sum)) +
  geom_line(aes(linetype = evolution, color = evolution), linewidth = 0.7) +
  theme_bw(base_size = 12) +
  ggh4x::facet_grid2(desc(lat_group)~climate, scales = "free_y", independent = "y") +
  labs(
    x = "Time",
    y = "Population Size",
...
ggsave(s6, file = paste0("pop_by_lat.pdf"), path = here::here("figs", "sup"), height = 15, width = 25)
```

```{r}
# plot the collapse points of the population
s7 <- ggplot(collapse_by_lat, aes(min_gen, mpa_size)) +
  ...
  facet_wrap(~desc(lat_group), ncol = 1)

ggsave(s7, file = paste0("by_lat_collapse.pdf"), path = here::here("figs", "sup"), height = 15, width = 10)
```
...
temps_fp <- aa_fp %>%
  select(mpa_size, climate, freq_avg, generation, fishing_pressure)
