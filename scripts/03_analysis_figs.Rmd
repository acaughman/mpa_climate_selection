...
collapse_outside <- line_df_outside_full %>%
  ...
  filter(generation > 30) %>% # filter generations when climate change causes declines
  filter((location_sum <= (100 * 2 * .05) & loc == "MPA" & mpa_size == "1 x 1") |(location_sum <= (100 * 2 * .05 * 3 * 3) & loc == "MPA" & mpa_size == "3 x 3") | (location_sum <= (100 * 2 * .05 * 6 * 6) & loc == "MPA" & mpa_size == "6 x 6") | (location_sum <= (100 * 2 * .05 * 6 * 6) & loc == "MPA" & mpa_size == "None") |(location_sum <= (100 * 2 * .05 * 10 * 10) & loc == "MPA" & mpa_size == "10 x 10") | (location_sum <= (100 * 2 * .05 * 10) & loc != "MPA")) %>% # calculate collapse rates in different MPAs and filter population
...
ggsave(f3, file = paste0("fig3.pdf"), path = here::here("figs"), height = 10, width = 12)
```
...
f6 <- ggplot(geno_lat, aes(generation, mpa_mean_freq)) +
  ...
  scale_linetype_manual(values = c("solid", "dashed", "dotted", "longdash", "solid", "solid"), labels = c("10 x 10", "6 x 6", "3 x 3", "1 x 1", "None", ""))

ggsave(f6, file = paste0("fig6.pdf"), path = here::here("figs"), height = 8, width = 15)
```
...
ggsave(f5, file = paste0("fig5.pdf"), path = here::here("figs"), height = 8, width = 15)
```

```{r}
# # plot the collapse points of the population
# f5 <- ggplot(collapse_full, aes(min_gen, mpa_size)) +
#   geom_point(aes(color = climate, shape = evolution), size = 3) +
#   theme_bw(base_size = 12) +
