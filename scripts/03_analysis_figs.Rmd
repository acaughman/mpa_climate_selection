...
outside_df <- data %>%
  filter((lat %in% c(49:51) & mpa_size == "3 x 3") | (lat %in% c(47:52) & mpa_size == "6 x 6") | (lat %in% c(46:55) & mpa_size == "10 x 10") | (lat %in% c(50) & mpa_size == "1 x 1") | lat %in% c(10, 90)) %>%
  filter((lon %in% c(9:11) & mpa_size == "3 x 3") | (lon %in% c(9:14) & mpa_size == "6 x 6") | (lon %in% c(5:14) & mpa_size == "10 x 10") | (lon %in% c(10) & mpa_size == "1 x 1")) %>% 
...
collapse_full <- line_df_mpa_full %>%
  ...
  filter((location_sum <= (100 * 2 * .01 * 3 * 3) & mpa_size == "3 x 3") | (location_sum <= (100 * 2 * .01 * 6 * 6) & mpa_size == "6 x 6") | (location_sum <= (100 * 2 * .01 * 10 * 10) & mpa_size == "10 x 10") | (location_sum <= (100 * 2 * .01) & mpa_size == "1 x 1")) %>% # calculate collapse rates in different MPAs
...
collapse_outside <- line_df_outside_full %>%
  ...
  filter(generation > 30) %>% # filter generations when climate change causes declines
  filter((location_sum <= (100 * 2 * .05) & loc == "MPA" & mpa_size == "1 x 1") |(location_sum <= (100 * 2 * .05 * 3 * 3) & loc == "MPA" & mpa_size == "3 x 3") |(location_sum <= (100 * 2 * .05 * 6 * 6) & loc == "MPA" & mpa_size == "6 x 6") |(location_sum <= (100 * 2 * .05 * 10 * 10) & loc == "MPA" & mpa_size == "10 x 10") | (location_sum <= (100 * 2 * .05) & loc != "MPA")) %>% # calculate collapse rates in different MPAs and filter population
...
f3.1 <- ggplot(point_sum, aes(generation, diff)) +
  geom_smooth(aes(color = climate), se=FALSE, method = "loess") +
  # geom_point() +
  theme_bw(base_size = 12) +
  labs(
    x = "Time",
    y = "Increase in Population Size",
    title = "Change in population size in the MPA when considering movement evolution",
    color = "Climate Scenario"
  ) +
  scale_color_manual(values = c("#21918c", "#5ec962", "#fde725", "#3b528b", "#440154")) +
...
```{r}
med = line_df_outside_full %>% 
  filter(mpa_size == "6 x 6")

# plot population through time in the Medium MPA
f5 <- ggplot(med, aes(generation, location_sum)) +
  geom_line(aes(linetype = evolution, color = evolution), linewidth = 0.7) +
  theme_bw(base_size = 12) +
  facet_wrap(climate~loc, ncol = 3, scales = "free_y") +
  labs(
    x = "Time",
    y = "Population Size",
    color = "Climate Scenario",
...
ggsave(f5, file = paste0("outside_pop.pdf"), path = here::here("figs"), height = 8, width = 15)
```

```{r}
# plot the collapse points of the population
f3.3 <- ggplot(collapse_outside, aes(min_gen, mpa_size)) +
  geom_point(aes(color = climate, shape = evolution), size = 3) +
  theme_bw(base_size = 12) +
  scale_color_manual(values = c("#21918c", "#5ec962", "#fde725", "#3b528b", "#440154")) +
  labs(
...
ggsave(f3.3, file = paste0("outside_collapse.pdf"), path = here::here("figs"), height = 8, width = 10)
```
...
# plot population through time in the Medium MPA
f6 <- ggplot(med2, aes(generation, location_sum)) +
  geom_line(aes(linetype = evolution, color = evolution), linewidth = 0.7) +
  theme_bw(base_size = 12) +
...
ggsave(f6, file = paste0("pop_by_lat.pdf"), path = here::here("figs"), height = 15, width = 25)
```

```{r}
# plot the collapse points of the population
f3.4 <- ggplot(collapse_by_lat, aes(min_gen, mpa_size)) +
  geom_point(aes(color = climate, shape = evolution), size = 3) +
  theme_bw(base_size = 12) +
  scale_color_manual(values = c("#21918c", "#5ec962", "#fde725", "#3b528b", "#440154")) +
  labs(
  ...
  facet_wrap(~lat_group, ncol = 1)

ggsave(f3.4, file = paste0("by_lat_collapse.pdf"), path = here::here("figs"), height = 15, width = 10)
```
